...
<xsl:stylesheet
	...
	<!-- For the case the dspace service is not running as root application. example: /dissertations -->
		...
		<xsl:variable name="fileData" select="//mets:fileGrp[@USE='CONTENT']"/>               
                <zip:archive>
                        <xsl:for-each select="$fileData/mets:file">                              
                                <zip:entry name="{mets:FLocat/@xlink:title}" src="{concat('cocoon:/', substring-after(mets:FLocat/@xlink:href, $context))}"/>
                        </xsl:for-each>
                </zip:archive>             
	</xsl:template>
